...
export const SidebarLists = () => {
  const { data: lists, isLoading: loadingLists, isError: errorLists } = useGetUserLists();
  ...
  return (
    <div>
      <h3 className="text-xs font-bold text-muted-foreground px-7 mb-3 mt-10">LISTAS</h3>
      <ul className="flex flex-col gap-y-1 px-3">
        {lists?.map((list) => (
          <Link key={list.id} href={`/dashboard/lists/${list.id}`} prefetch={true}>
            <div
              className={cn(
